...
(function () {
  ...
  const data = {
    dry: {
      icon: 'ph-drop', title: 'Сухость и стянутость',
      text: 'Коже не хватает влаги и липидного барьера. Начинаем с глубокого увлажнения и восстановления защитного слоя.',
      proc: 'Биоревитализация', price: 'от 4 500₽',
    },
    acne: {
      icon: 'ph-bandaids', title: 'Акне и воспаления',
      text: 'Работаем с причиной, а не только со следствием. Сначала снимаем воспаление, затем нормализуем работу пор.',
      proc: 'Чистка + пилинг', price: 'от 2 800₽',
    },
    pigment: {
      icon: 'ph-sun', title: 'Пигментация',
      text: 'Неравномерный тон и пятна осветляем курсом пилингов и поддерживаем результат домашним уходом с SPF.',
      proc: 'Срединный пилинг', price: 'от 3 200₽',
    },
    aging: {
      icon: 'ph-clock-counter-clockwise', title: 'Морщины и тонус',
      text: 'Стимулируем выработку коллагена и подтягиваем овал. Аппаратные методики плюс инъекционная поддержка.',
      proc: 'RF-лифтинг / мезотерапия', price: 'от 3 500₽',
    },
    pores: {
      icon: 'ph-circles-three', title: 'Расширенные поры',
      text: 'Очищаем и сужаем поры, выравниваем рельеф. Мягкая чистка в сочетании с обновляющими процедурами.',
      proc: 'Ультразвуковая чистка', price: 'от 3 200₽',
    },
    dull: {
      icon: 'ph-sparkle', title: 'Тусклый цвет лица',
      text: 'Возвращаем сияние: обновляем верхний слой кожи и насыщаем её влагой и витаминами.',
      proc: 'Газожидкостный пилинг', price: 'от 3 600₽',
    },
  };
  const iconEl = document.getElementById('concern-icon');
  const titleEl = document.getElementById('concern-title');
  const textEl = document.getElementById('concern-text');
  const procEl = document.getElementById('concern-proc');
  const priceEl = document.getElementById('concern-price');
  const apply = (key) => {
    const d = data[key];
    if (!d) return;
    iconEl.className = 'ph ' + d.icon;
    titleEl.textContent = d.title;
    textEl.textContent = d.text;
    procEl.textContent = d.proc;
    priceEl.textContent = d.price;
  };
  grid.querySelectorAll('.concern').forEach((btn) => {
    btn.addEventListener('click', () => {
      grid.querySelectorAll('.concern').forEach((b) => b.classList.remove('is-active'));
