...
(function () {
  ...
  const sumEl = document.getElementById('debt-sum');
  const verdictEl = document.getElementById('debt-verdict');
  if (!debt) return;
  const fmt = (n) => Math.round(n).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ') + ' ₽';
  const update = () => {
    const v = parseInt(debt.value, 10);
    label.textContent = fmt(v);
    sumEl.textContent = fmt(v);
    let verdict;
    if (v < 250000) verdict = 'Сумма небольшая — поможем договориться с кредиторами или подберём другой путь.';
    else if (v <= 1000000) verdict = 'Подходит упрощённое банкротство через МФЦ — без суда и бесплатно.';
    else verdict = 'Подходит судебное банкротство — спишем долг законно и полностью.';
