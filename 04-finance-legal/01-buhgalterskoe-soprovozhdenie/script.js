...
// ---------- Savings calculator ----------
(function () {
  const emp = document.getElementById('calc-emp');
  const empLabel = document.getElementById('calc-emp-label');
  const typeBtns = document.querySelectorAll('.calc-type');
  const tariffEl = document.getElementById('calc-tariff');
  const saveEl = document.getElementById('calc-save');
  if (!emp || !tariffEl) return;

  const inhouse = 65000;
  let base = 4900;
  const fmt = (n) => Math.round(n).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ' ') + ' ₽';
  const update = () => {
    const e = parseInt(emp.value, 10);
    empLabel.textContent = e;
    const tariff = base + e * 600;
    tariffEl.textContent = fmt(tariff);
    saveEl.textContent = fmt(Math.max(0, (inhouse - tariff) * 12));
  };
  emp.addEventListener('input', update);
